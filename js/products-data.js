// Product data dengan pageUrl
const productData = {
  categories: [
    { id: 1, name: "Semua Produk" },
    { id: 2, name: "Streaming" },
    { id: 3, name: "Musik" },
    { id: 4, name: "Editing" }
  ],
  products: [
    // 🔥 STREAMING
    {
      id: 1,
      name: "NETFLIX",
      category: "Streaming",
      price: 18000,
      originalPrice: 49000,
      image: "image/netflix.png",
      discount: 63,
      isPopular: true,
      isFlashSale: true,
      pageUrl: "Netflix.html",
      description: "Layanan streaming film dan series terpopuler",
      features: ["Aktif Instan", "Garansi 30 Hari", "Support 24/7"],
      stock: 100,
      sold: 2500
    },
    {
      id: 2,
      name: "Vidio",
      category: "Streaming",
      price: 18000,
      originalPrice: 35000,
      image: "image/vidio.png",
      discount: 49,
      isPopular: true,
      isFlashSale: true,
      pageUrl: "Vidio.html",
      description: "Streaming film dan series lokal terbaik",
      features: ["Aktif Instan", "Garansi 30 Hari", "Content Lokal"],
      stock: 80,
      sold: 1800
    },
    {
      id: 3,
      name: "DISNEY+",
      category: "Streaming",
      price: 23000,
      originalPrice: 55000,
      image: "image/disneyplus.png",
      discount: 58,
      isPopular: false,
      isFlashSale: true,
      pageUrl: "Disney.html",
      description: "Disney, Marvel, Star Wars dalam satu platform",
      features: ["Aktif Instan", "Garansi 30 Hari", "All Content"],
      stock: 60,
      sold: 1200
    },
    {
      id: 4,
      name: "YOUTUBE",
      category: "Streaming",
      price: 3500,
      originalPrice: 65000,
      image: "image/youtube.png",
      discount: 95,
      isPopular: true,
      isFlashSale: true,
      pageUrl: "YouTube.html",
      description: "YouTube Premium tanpa iklan",
      features: ["No Ads", "Background Play", "Download Video"],
      stock: 200,
      sold: 3500
    },
    {
      id: 5,
      name: "AMAZON PRIME",
      category: "Streaming",
      price: 6000,
      originalPrice: 12000,
      image: "image/prime-video.png",
      discount: 50,
      isPopular: false,
      isFlashSale: true,
      pageUrl: "AmazonPrime.html",
      description: "Amazon Prime Video dengan konten eksklusif",
      features: ["Aktif Instan", "Garansi 30 Hari", "Original Content"],
      stock: 50,
      sold: 800
    },
    {
      id: 6,
      name: "IQIYI",
      category: "Streaming",
      price: 6000,
      originalPrice: 15000,
      image: "image/iqiyi.png",
      discount: 60,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "IQIYI.html",
      description: "Streaming drama dan film Asia",
      features: ["Aktif Instan", "Garansi 30 Hari", "Asian Content"],
      stock: 40,
      sold: 600
    },
    {
      id: 7,
      name: "DRAMABOX",
      category: "Streaming",
      price: 18000,
      originalPrice: 30000,
      image: "image/dramabox.png",
      discount: 40,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Dramabox.html",
      description: "Platform streaming khusus drama",
      features: ["Aktif Instan", "Garansi 30 Hari", "Drama Collection"],
      stock: 30,
      sold: 450
    },
    {
      id: 8,
      name: "LOKLOK",
      category: "Streaming",
      price: 21000,
      originalPrice: 45000,
      image: "image/loklok.png",
      discount: 53,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Loklok.html",
      description: "Streaming film pendek dan konten viral",
      features: ["Aktif Instan", "Garansi 30 Hari", "Short Videos"],
      stock: 35,
      sold: 500
    },
    {
      id: 9,
      name: "VIU",
      category: "Streaming",
      price: 2000,
      originalPrice: 5000,
      image: "image/viu.png",
      discount: 60,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "VIU.html",
      description: "Streaming drama Korea dan Asia",
      features: ["Aktif Instan", "No Ads", "Download Feature"],
      stock: 70,
      sold: 1500
    },
    {
      id: 10,
      name: "HBO GO",
      category: "Streaming",
      price: 15500,
      originalPrice: 50000,
      image: "image/hbo-go.png",
      discount: 69,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "HBOGO.html",
      description: "HBO dengan series premium",
      features: ["Aktif Instan", "Garansi 30 Hari", "Premium Series"],
      stock: 25,
      sold: 400
    },
    {
      id: 11,
      name: "WETV",
      category: "Streaming",
      price: 13000,
      originalPrice: 30000,
      image: "image/wetv.png",
      discount: 57,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "WeTV.html",
      description: "Tencent Video international",
      features: ["Aktif Instan", "Garansi 30 Hari", "Chinese Content"],
      stock: 45,
      sold: 700
    },
    {
      id: 12,
      name: "HBO GO MAX",
      category: "Streaming",
      price: 14500,
      originalPrice: 15000,
      image: "image/hbo-go-max-com.png",
      discount: 3,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "HBOGOMax.html",
      description: "HBO Max dengan konten lengkap",
      features: ["Aktif Instan", "Garansi 30 Hari", "Max Content"],
      stock: 20,
      sold: 300
    },
    {
      id: 13,
      name: "BSTATION",
      category: "Streaming",
      price: 6000,
      originalPrice: 35000,
      image: "image/bstation.png",
      discount: 83,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Bstation.html",
      description: "Platform streaming anime dan manga",
      features: ["Aktif Instan", "Garansi 30 Hari", "Anime Content"],
      stock: 55,
      sold: 900
    },
    {
      id: 14,
      name: "YOUKOU",
      category: "Streaming",
      price: 10000,
      originalPrice: 30000,
      image: "image/youkou.png",
      discount: 67,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Youkou.html",
      description: "Youku dengan konten China terbaru",
      features: ["Aktif Instan", "Garansi 30 Hari", "Chinese Drama"],
      stock: 40,
      sold: 650
    },

    // 🔥 MUSIK
    {
      id: 15,
      name: "SPOTIFY",
      category: "Musik",
      price: 18000,
      originalPrice: 75000,
      image: "image/spotify.png",
      discount: 76,
      isPopular: true,
      isFlashSale: false,
      pageUrl: "Spotify.html",
      description: "Spotify Premium dengan fitur lengkap",
      features: ["No Ads", "Download Music", "High Quality"],
      stock: 150,
      sold: 2800
    },


    // 🔥 EDITING
    {
      id: 17,
      name: "CANVA",
      category: "Editing",
      price: 2000,
      originalPrice: 10000,
      image: "image/canva.png",
      discount: 80,
      isPopular: true,
      isFlashSale: false,
      pageUrl: "Canva.html",
      description: "Canva Pro untuk desain grafis",
      features: ["All Elements", "Premium Templates", "No Watermark"],
      stock: 200,
      sold: 4200
    },
    {
      id: 18,
      name: "CAPCUT",
      category: "Editing",
      price: 10000,
      originalPrice: 10000,
      image: "image/capcut.png",
      discount: 0,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Capcut.html",
      description: "Capcut Pro untuk editing video",
      features: ["No Watermark", "All Effects", "Premium Features"],
      stock: 120,
      sold: 1900
    },
    {
      id: 19,
      name: "REMINI",
      category: "Editing",
      price: 8000,
      originalPrice: 12000,
      image: "image/remini.png",
      discount: 33,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Remini.html",
      description: "Remini untuk enhacement foto",
      features: ["AI Enhancement", "HD Quality", "All Features"],
      stock: 80,
      sold: 1400
    },
    {
      id: 20,
      name: "MEITU",
      category: "Editing",
      price: 28000,
      originalPrice: 55000,
      image: "image/meitu.png",
      discount: 49,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Meitu.html",
      description: "Meitu untuk editing foto profesional",
      features: ["All Filters", "Premium Tools", "No Watermark"],
      stock: 60,
      sold: 1100
    },
    {
      id: 21,
      name: "PICSART",
      category: "Editing",
      price: 8000,
      originalPrice: 8000,
      image: "image/picsart.png",
      discount: 0,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "Picsart.html",
      description: "Picsart Gold untuk editing lengkap",
      features: ["All Stickers", "Premium Filters", "No Ads"],
      stock: 100,
      sold: 1800
    },

    {
      id: 23,
      name: "IBIS PAINT X",
      category: "Editing",
      price: 18000,
      originalPrice: 30000,
      image: "image/ibispaint.png",
      discount: 40,
      isPopular: false,
      isFlashSale: false,
      pageUrl: "IbisPaint.html",
      description: "Ibis Paint X untuk digital art",
      features: ["All Brushes", "Premium Tools", "No Ads"],
      stock: 50,
      sold: 900
    },
    {
      id: 24,
      name: "Alight Motion",
      category: "Editing",
      price: 3000,
      originalPrice: 10000,
      image: "image/alight-motion.png",
      discount: 70,
      isPopular: true,
      isFlashSale: false,
      pageUrl: "AlightMotion.html",
      description: "Alight Motion Pro untuk motion graphics",
      features: ["All Effects", "No Watermark", "Premium Features"],
      stock: 180,
      sold: 3500
    },

  ]
};